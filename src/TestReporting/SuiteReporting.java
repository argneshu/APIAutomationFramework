...
public class SuiteReporting extends Reporting {
	...
	public void setSuiteNameWithIp(){
		this.suiteName =  this.suiteName+"_"+GlobalVar.serverIP;
	...
	public void createConsolidateHeader() {
		try {
			...
			consolidatePrintHtml
					.println("<td width='30%' bgcolor='#CCCCFF'align='center'><b><font color='#000000' face='Tahoma' size='2'>TestCase Percent</font></b></td>");
			consolidatePrintHtml.println("</tr>");
		} catch (Exception ex) {
			ex.printStackTrace();
		}
	...
	public void writeFileInConsolidate() {//add argument
		try {
			openConsolidateFile();
	...
	public void checkTestCaseStatus(File testCaseFile) {//change argument file to string
		String Status = "PASS";
		String[] line3;
		String sPercent = "";
		...
		try {
			String resultLink = "<a href=\"" + "TestCase/"
					+ testCaseFile + "\">TestCaseResultFile</a>";
			...
			addTestToConsolidate(Status, getTestCaseName(testCaseFile.getName()), resultLink,
					sPercent);
		} catch (Exception ex) {
			ex.printStackTrace();
		}
	...
	public void addTestToConsolidate(String strPassFail, String testCaseName,
			String ResultFileLink, String sPassPercent) {
...
//		SuccessRate = (int_Pass * 100 / (testcasetotal));
	...
	public File createTempDir() throws IOException {
		...
		do {
			...
			//String dirName = "ShopClues_Results_" + getTime();
			String dirName = "Zopper_Results_"
					+ getTime();
